...
// DoubleJumpPickup represents a placed double jump pickup that floats and can be triggered by the player.
type DoubleJumpPickup struct {
...
// NewDoubleJumpPickup creates a DoubleJumpPickup at world pixel (x,y). spritePath may be empty.
func NewDoubleJumpPickup(x, y float32, spritePath string, onPickup func()) *DoubleJumpPickup {
	di := &DoubleJumpPickup{
		X:          x,
		Y:          y,
		Disabled:   false,
...
func (d *DoubleJumpPickup) Update(p *Player) {
...
func (d *DoubleJumpPickup) Draw(screen *ebiten.Image, camX, camY, zoom float64) {
	if d == nil || d.Disabled {
		return
	}
