package module

import (
	"fmt"

	"github.com/d5/tengo/v2"
	"github.com/milk9111/sidescroller/ecs"
	"github.com/milk9111/sidescroller/ecs/component"
)

func ParticleEmitterModule() Module {
	return Module{
		Name: "particle_emitter",
		Build: func(world *ecs.World, _ map[string]ecs.Entity, _ ecs.Entity, target ecs.Entity) map[string]tengo.Object {
			values := map[string]tengo.Object{}

			values["disable"] = &tengo.UserFunction{Name: "disable", Value: func(args ...tengo.Object) (tengo.Object, error) {
				emitter, ok := ecs.Get(world, target, component.ParticleEmitterComponent.Kind())
				if !ok || emitter == nil {
					return tengo.FalseValue, fmt.Errorf("particle emitter component not found for entity %v", target)
				}

				emitter.Disabled = true
				return tengo.TrueValue, nil
			}}

			values["enable"] = &tengo.UserFunction{Name: "enable", Value: func(args ...tengo.Object) (tengo.Object, error) {
				emitter, ok := ecs.Get(world, target, component.ParticleEmitterComponent.Kind())
				if !ok || emitter == nil {
					return tengo.FalseValue, fmt.Errorf("particle emitter component not found for entity %v", target)
				}

				emitter.Disabled = false
				emitter.HasEmittedAtLeastOnce = false
				return tengo.TrueValue, nil
			}}

			return values
		},
	}
}
